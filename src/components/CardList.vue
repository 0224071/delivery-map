...
export default {
  ...
  computed: {
    dataFiltered() {
      ...
      let fp_data = this.fp_markers.$attrs.info;
      // if (!fp_data) return [];
      // return fp_data.filter((item) => {
      //   return this.filterString
      //     ? item.name.includes(this.filterString) ||
      //         this.menuFiltered(item.menu_list, this.filterString)
      //     : true;
      // });
    },
  },
